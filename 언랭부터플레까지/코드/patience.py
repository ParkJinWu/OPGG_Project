...
class Patience :    #끈기점수를 연산하기 위한 클래스. 필드 key, summonerName이 필요해 클래스를 선언하였다.
    ...
    def get_result(self) :    #최종 결과(I or R)을 반환한다.
        ...
        print("%s님의 끈기점수는 %f입니다."%(self.summonerName, patience_score))
        
        with open('gm_result_list.txt', 'rb') as f:
            gm_result_list = pickle.load(f) 

        x = np.array(gm_result_list)
        meanValue = np.mean(x)

        if patience_score >= meanValue :
            return 'I'
        else :
            return 'R'
